...
class FBevelCubeGeometry;
class FBevelCubeBuilder;

/**
 * 倒角(圆角)立方体生成器
 * 使用程序化网格生成圆角立方体几何体，支持可配置的大小、倒角和分段数
 */
...
public:
    ...
    float CubeBevelSize = 10.0f;

    /** 倒角的分段数，影响倒角的平滑度 */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "BevelCube|Parameters", 
        meta = (ClampMin = "1", ClampMax = "10", UIMin = "1", UIMax = "10", DisplayName = "Bevel Sections"))
...
private:
	...
	/** 几何体生成器 */
    TUniquePtr<FBevelCubeBuilder> GeometryBuilder;
...
public:
	...
	//~ Begin Constructor
    explicit FBevelCubeBuilder(const FBuildParameters& InParams);

	//~ Begin Public Methods
    /** 生成圆角立方体的几何体 */
    bool Generate(FBevelCubeGeometry& OutGeometry);
...
private:
	...
	/** 生成主要面 */
    void GenerateMainFaces(FBevelCubeGeometry& Geometry);
	
	/** 生成边缘倒角 */
    void GenerateEdgeBevels(FBevelCubeGeometry& Geometry, const TArray<FVector>& CorePoints);
	
	/** 生成角落倒角 */
    void GenerateCornerBevels(FBevelCubeGeometry& Geometry, const TArray<FVector>& CorePoints);

	//~ Begin Vertex Management
	/** 获取或添加顶点，确保顶点不重复 */
    int32 GetOrAddVertex(FBevelCubeGeometry& Geometry, const FVector& Pos, const FVector& Normal, const FVector2D& UV);
	
	/** 添加新顶点到几何体 */
    int32 AddVertex(FBevelCubeGeometry& Geometry, const FVector& Pos, const FVector& Normal, const FVector2D& UV);

	//~ Begin Geometry Helper Methods
	/** 添加四边形（两个三角形） */
    void AddQuad(FBevelCubeGeometry& Geometry, int32 V1, int32 V2, int32 V3, int32 V4);
	
	/** 添加三角形 */
    void AddTriangle(FBevelCubeGeometry& Geometry, int32 V1, int32 V2, int32 V3);
	
	/** 生成矩形的顶点 */
    TArray<FVector> GenerateRectangleVertices(const FVector& Center, const FVector& SizeX, const FVector& SizeY) const;
	
	/** 生成四边形面 */
    void GenerateQuadSides(FBevelCubeGeometry& Geometry, const TArray<FVector>& Verts, const FVector& Normal, const TArray<FVector2D>& UVs);
    ...
    /** 生成边缘倒角的条带 */
    void GenerateEdgeStrip(FBevelCubeGeometry& Geometry, const TArray<FVector>& CorePoints, int32 Core1Idx, int32 Core2Idx, const FVector& Normal1, const FVector& Normal2);
    ...
    /** 生成角落倒角的三角形 */
    void GenerateCornerTriangles(FBevelCubeGeometry& Geometry, const TArray<TArray<int32>>& CornerVerticesGrid, int32 Lat, int32 Lon, bool bSpecialOrder);
