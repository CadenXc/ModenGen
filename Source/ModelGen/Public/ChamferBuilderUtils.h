...
/**
 * @file BevelBuilderUtils.h (deprecated alias)
 * @brief 倒角生成的通用工具（边倒角、角倒角、环形倒角），面向各 Builder 复用。
 *
                                     ...
                                     int32 sections)
{
    ...
    for (int32 s = 0; s <= sections; ++s)
    {
        ...
        const int32 vStart = builder.GetOrAddVertex(posStart, n, uvStart);
        const int32 vEnd   = builder.GetOrAddVertex(posEnd,   n, uvEnd);

        if (s > 0 && prevStart != INDEX_NONE && prevEnd != INDEX_NONE)
        {
            builder.AddQuad(prevStart, prevEnd, vEnd, vStart);
        }
                                               ...
                                               bool bSpecialOrder = false)
{
    ...
    for (int32 lat = 0; lat <= sections; ++lat)
    {
        for (int32 lon = 0; lon <= sections - lat; ++lon)
        {
            ...
            grid[lat][lon] = builder.GetOrAddVertex(pos, n, uv);
        }
    }
