...
#include "Materials/Material.h" // 包含 UMaterial 类所需
#include "UObject/ConstructorHelpers.h" // 包含 ConstructorHelpers 类所需
...
AChamferCube::AChamferCube()
{
    ...
    // 设置立方体生成的相关默认参数
    CubeSize = 100.0f;          // 立方体总大小
    CubeChamferSize = 20.0f;    // 倒角大小
    ChamferSections = 8;        // 倒角分段数（值越大，倒角越平滑）
...
void AChamferCube::GenerateChamferedCube(float Size, float ChamferSize, int32 Sections)
{
    ...
    auto GetOrAddVertex = [&](const FVector& Pos, const FVector& Normal, const FVector2D& UV) -> int32
        {
            ...
            if (FoundIndex) // 如果找到
            {
                return *FoundIndex; // 返回已存在的索引
            }

            // 如果未找到，则添加新顶点
            int32 NewIndex = AddVertexInternal(Vertices, Normals, UV0, VertexColors, Tangents, Pos, Normal, UV);
            UniqueVerticesMap.Add(Pos, NewIndex); // 将新顶点的位置和索引添加到映射中
            return NewIndex; // 返回新顶点的索引
        };
    ...
    AddQuadInternal(Triangles, VxNyNz, VxNyPz, VxPyPz, VxPyNz); // +X 面缠绕顺序 (外向)
    ...
    AddQuadInternal(Triangles, NxNyNz, NxPyNz, NxPyPz, NxNyPz); // -X 面缠绕顺序 (外向)
    ...
    AddQuadInternal(Triangles, PyNxNz, PyPxNz, PyPxPz, PyNxPz); // +Y 面缠绕顺序 (外向)
    ...
    AddQuadInternal(Triangles, NyNxNz, NyNxPz, NyPxPz, NyPxNz); // -Y 面缠绕顺序 (外向)
    ...
    AddQuadInternal(Triangles, PzNxNy, PzNxPy, PzPxPy, PzPxNy); // +Z 面缠绕顺序 (外向)
    ...
    AddQuadInternal(Triangles, NzNxNy, NzPxNy, NzPxPy, NzNxPy); // -Z 面缠绕顺序 (外向)
    ...
    {
        ProceduralMesh->SetMaterial(0, MaterialFinder.Object); // 设置第一个网格节的材质
    }
    else
    {
